...
def datasets():
    st.subheader("📚 2.1.1 Datasets Disponíveis")
    ...
    if df_meta.empty:
        st.warning("Nenhum dataset encontrado. Adicione arquivos .csv ou .tsv na pasta 'data' e clique em 'Atualizar Datasets'.")
        return
    ...
    if df is None:
        error_message = f"Não foi possível carregar o dataset '{dataset_name}'."
        st.error(error_message)
        return

    st.success(f"Dataset '{dataset_name}' carregado com sucesso!")

    st.subheader("📊 2.1.2 Análise e Agrupamento de Atributos")
